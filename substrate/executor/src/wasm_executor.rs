...
impl_function_executor!(this: FunctionExecutor<'e, E>,
	...
	ext_sandbox_memory_get(memory_idx: u32, offset: u32, buf_ptr: *mut u8, buf_len: usize) => {
		let dst_memory = this.sandbox_store.memory(memory_idx).ok_or_else(|| DummyUserError)?;

		let tmp: Vec<u8> = dst_memory.get(offset, buf_len as usize).map_err(|_| DummyUserError)?;
		this.memory.set(buf_ptr, &tmp).map_err(|_| DummyUserError)?;

		Ok(())
	},
	ext_sandbox_memory_set(memory_idx: u32, offset: u32, val_ptr: *const u8, val_len: usize) => {
		let dst_memory = this.sandbox_store.memory(memory_idx).ok_or_else(|| DummyUserError)?;

		let tmp = this.memory.get(offset, val_len as usize).map_err(|_| DummyUserError)?;
		dst_memory.set(val_ptr, &tmp).map_err(|_| DummyUserError)?;
